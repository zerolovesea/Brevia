...
def complete(payload, prompt, json_mode=False):
    ...
    request = urllib.request.Request(endpoint, json.dumps(body).encode(), headers=headers, method="POST")
    try:
        ...
        raise ValueError(f"LLM request failed ({error.code}): {detail[:500]}") from error
    content = data.get("message", {}).get("content")
    if not content and data.get("choices"):
        content = data["choices"][0].get("message", {}).get("content")
    if not content and isinstance(data.get("content"), list):
        content = "".join(item.get("text", "") for item in data["content"] if item.get("type", "text") == "text")
