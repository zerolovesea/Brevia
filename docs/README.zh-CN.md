...
<p align="center"><strong>极简设计，本地部署的 AI 会议助手。</strong><br />AI 辅助笔记 · 实时转写 · 多语言 · 说话人识别 · 可审阅总结 — 音频不出本机。</p>
...
言录是一款桌面端 AI 会议助手，把会议里最耗时间的部分——记录、整理、复盘——交给设备上的 AI。它同时录制麦克风和系统音频，实时生成字幕，会后自动整理成结构化笔记；会议进行中，AI 辅助笔记会及时提示值得记录的内容。所有语音识别都在本机运行，录音、文字稿、说话人档案默认保存在你自己的电脑上。
...
AI 辅助笔记复用已有的总结模型配置。使用远程服务时，只会发送逐字稿文本与当前笔记上下文，音频始终留在本机。

![AI 辅助笔记](assets/tour/zh/AI辅助笔记.png)
