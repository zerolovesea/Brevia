...
<p align="center"><strong>Минималистичный ИИ-ассистент встреч с приоритетом локальной работы.</strong><br />AI Assist Notes · живая транскрипция · многоязычность · идентификация говорящих · проверяемые ИИ-резюме — аудио не покидает ваше устройство.</p>
...
### AI Assist Notes с проверкой каждого предложения

AI Assist следит за живой транскрипцией и может выделять решения, задачи, ключевые числа, риски, вопросы и смену тем. Выберите запуск по запросу, ненавязчивые подсказки или автоматическую организацию. Все предложения можно проверить: добавляйте в заметки только полезное и продолжайте писать в форматированном тексте или Markdown.

AI Assist использует существующую настройку модели резюме. При работе с удалённым провайдером отправляются только текст транскрипции и текущий контекст заметок; аудио никогда не покидает устройство.
...
Загружаемые модели охватывают потоковое ASR, офлайн-уточнение, восстановление пунктуации, детекцию речевой активности, диаризацию, эмбеддинги говорящего и разделение источников. Комбинируйте по языку и точности — всё работает на вашем устройстве.
...
| --- | --- | --- |
| Потоковое ASR | Zipformer (zh / en / fr / ko / многоязычный), Nemotron 3.5 | 30+ |
| Уточняющее ASR | Qwen3-ASR 0.6B / 1.7B, Whisper Large v3, FunASR Nano | Многоязычные |
...
- Спасибо авторам и сопровождающим моделей, чьи загружаемые артефакты заявлены в [`backend/models.json`](../backend/models.json), включая Zipformer, Whisper, Qwen3-ASR, FunASR, Pyannote, 3D-Speaker, Silero, Spleeter и Tencent Hy-MT2.
