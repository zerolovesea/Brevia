...
Модели распознавания, обработки, ИИ-заметок и перевода субтитров загружаются по требованию из **Настройки → Библиотека моделей**; детекция речевой активности, разделение говорящих и голосовые отпечатки входят в комплект. Манифест находится в [`backend/models.json`](../backend/models.json).
...
Артефакты попадают в `dist/`. Каждая сборка для платформы включает нативный Python-воркер с детекцией речевой активности, разделением говорящих и голосовыми отпечатками; модели распознавания, обработки, ИИ-заметок и перевода остаются загрузками по требованию.
...
Зависит от того, что вы установите. Типовая конфигурация (распознавание фраз + обработка) занимает 1–2 ГБ; детекция речевой активности, разделение говорящих и голосовые отпечатки входят в комплект. Самая компактная модель распознавания занимает около 487 МБ; крупные модели превышают 1 ГБ.
</details>
