...
| --- | --- | --- |
| 中国語・広東語 | FunASR Nano int8 | 中国語とその方言で最も高精度 |
| 日本語・韓国語 | Qwen3-ASR 0.6B int8 | 選択可能なモデルで日韓両方をカバーする唯一のもの |
| 英語・スペイン語・フランス語・ドイツ語・ロシア語・混在言語 | Parakeet TDT 0.6B v3 | 25 の欧州言語を 1 モデルでカバーし、句読点とタイムスタンプも付与 |
| その他の言語 | Qwen3-ASR 0.6B int8 | 残りのモデルで最も広い言語カバレッジ |
...
録音は **Silero VAD による分割 → 1 回のオフライン認識 → 完全な字幕** という流れです。発話の区切り（中国語 0.7 秒、その他の言語 0.8 秒）ごとに文が確定し、連続発話は 30 / 20 秒で区切られます（`vad` で調整可能）。VAD の 1 区間に複数の文が含まれることがあり、隣接する文は 1 段落にまとめます（中国語 約 110 字・上限 150 字、ラテン文字 約 280 字・上限 380 字）。VAD の端点は段落境界では**ありません**——本当に長い無音（1.2 秒以上）だけが新しい段落を始め、目標に満たない段落は最大 8 秒で確定します。切れ目では次のデコードが 400 ミリ秒さかのぼり、継ぎ目の重複を除去します。停止時は最後の文まで処理し、認識に失敗しても元の音声は保持されます。
...
| 種類 | 代表モデル | 言語 |
| --- | --- | --- |
| 整句認識 / 会議後の高精度化 | FunASR Nano int8、Qwen3-ASR 0.6B、Parakeet TDT 0.6B v3 | 中国語 / 多言語 / 欧州 25 言語 |
...
インストールするモデルによります。典型的な構成（整句認識 + 高精度化 + 話者分離）で 1–2 GB。最小の認識モデルは約 487 MB、大きなモデルは 1 GB 超。
</details>
