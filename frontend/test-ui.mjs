...
for (const locale of ['zh', 'en', 'es', 'ja', 'ko', 'fr', 'de', 'ru']) for (const id of ['qwen3-asr-0.6b-int8', 'funasr-nano-int8', 'whisper-large-v3']) assert.ok(localeContext.window.BreviaLocaleData.refinedModelOptionTags[locale][id]?.length, `${locale} tags for ${id}`);
...
assert.match(text(js), /refined: DEFAULT_REFINED_MODEL_ID/);
...
assert.match(summaryHtml, /data-download-summary-model="qwen3\.5-4b-q4km"/);
