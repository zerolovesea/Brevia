...
assert.deepEqual(Array.from(localeContext.window.BreviaLocaleData.appCopy.modalCopy.zh.models.items.slice(10, 15), ([, name]) => name), ['Qwen3-ASR', 'FireRedASR2 CTC', 'FunASR Nano int8', 'Whisper Turbo', 'Qwen3-ASR 1.7B int8']);
...
assert.match(text(js), /type="password"[^>]*value="\$\{escapeHtml\(apiKey\)\}"/);
assert.doesNotMatch(text(js), /已安全保存，留空表示不修改/);
...
assert.match(text(electronMain), /ipcMain\.handle\('app\.open-releases'/);
assert.match(text(app), /window\.brevia\?\.openReleases\(\)/);
...
assert.match(text(electronMain), /ipcMain\.handle\('secret\.get'/);
...
assert.match(text(css), /\.meeting-menu,.meeting-rename-menu,.meeting-category-menu\{(?=[^}]*position:fixed)(?=[^}]*z-index:50)/);
...
assert.match(text(electronMain), /ipcMain\.handle\('permissions\.request-microphone'/);
assert.match(text(electronMain), /ipcMain\.handle\('metrics\.record'/);
assert.match(text(electronMain), /error\.code === 'EPIPE'/);
assert.match(text(electronMain), /resetOnboarding/);
assert.match(text(js), /localStorage\.removeItem\('brevia-onboarding-complete'\)/);
assert.match(text(electronMain), /getMediaAccessStatus\('microphone'\) === 'not-determined'/);
assert.match(text(js), /function openInitialPermissions/);
assert.match(text(js), /navigator\.mediaDevices\.getDisplayMedia/);
assert.match(text(js), /status\.microphone === 'not-determined' \|\| status\.screen === 'not-determined'/);
assert.match(text(js), /const requiredModelsForLanguage/);
assert.match(text(js), /'silero-vad', 'punct-ct-transformer-zh-en-int8', refined, \.\.\.diarization\.split\('\|'\), 'gtcrn-live-denoiser', 'spleeter-2stems-fp16'/);
