...
assert.match(text(css), /\.detail-refine \.flow-select-options\{[^}]*width:min\(20rem,100vw - 48px\)/);
...
assert.match(text(app), /const tags = refinedModelOptionTags\[locale\]/);
assert.match(text(i18nData), /const refinedModelOptionTags/);
assert.match(text(i18nData), /Chinese-English mixed/);
for (const locale of ['zh', 'en', 'es', 'ja', 'ko', 'fr', 'de', 'ru']) for (const id of ['qwen3-asr-0.6b-int8', 'funasr-nano-int8', 'qwen3-asr-1.7b-int8', 'whisper-large-v3']) assert.ok(localeContext.window.BreviaLocaleData.refinedModelOptionTags[locale][id]?.length, `${locale} tags for ${id}`);
