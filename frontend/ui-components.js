...
/** 渲染详情页 tabbar 右侧的精修状态控件。翻译与重新精修各自独立，避免一次选择触发另一项任务。@param {object} d 详情数据。@returns {string} 标记。 */
function renderRefineStatus(d) {
  ...
  const translateIcon = `<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M2.5 3.5h7M6 2v1.5c0 3-1.5 5.4-4 6.7M3.7 7.8c.8 1 1.8 1.8 3 2.3M9.5 12.5l2-5 2 5M10.3 10.7h2.4"/></svg>`;
  const refineIcon = `<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M13 7.5A5.2 5.2 0 1 1 11.4 4M13 2.8v3.1H9.9M10.8 11.7l.5 1.3.5-1.3 1.3-.5-1.3-.5-.5-1.3-.5 1.3-1.3.5z"/></svg>`;
  const translationMenu = `<span class="detail-translation-action"><button class="detail-icon-action" data-detail-translation-toggle type="button" aria-label="${t('翻译')}" title="${t('翻译')}" aria-expanded="false"${d.translationPending ? ' disabled' : ''}>${translateIcon}</button><div class="detail-translation-menu" hidden>${BreviaI18n.languageOptions(locale, t).slice(1).map(([code, name]) => `<button type="button" data-detail-translation="${escapeHtml(code)}">${escapeHtml(name)}</button>`).join('')}</div></span>`;
  const refineMenu = `<span class="refine-wrap"><button class="detail-icon-action" data-refine-more type="button" aria-label="${t('重新精修')}" title="${t('重新精修')}" aria-expanded="false">${refineIcon}</button><div class="refine-menu" hidden>${languageSelect}${renderRefineModelRow(d.refinedModelOptions, d.refinedModelId)}<label class="refine-menu-speakers"><span>${t('会议人数')}</span><input type="number" min="1" step="1" inputmode="numeric" data-refine-num-speakers placeholder="${t('留空自动识别')}" /></label><button type="button" data-refine-action="${d.hasRefined ? 're-refine' : 'start'}">${d.hasRefined ? t('重新精修') : t('开始精修')}</button></div></span>`;
  if (!d.hasRefined) return `${translationMenu}${refineMenu}`;
  return `<span class="refine-state is-done">${checkIconSvg} ${t('已精修')}${refineAppliedModelSuffix(d)}</span>${translationMenu}${refineMenu}`;
}
...
function renderDetailTabbar() {
  ...
  return `<div class="tabbar"><div class="tabbar-tabs"><button class="tab${detailActiveTab === 'notes' ? ' active' : ''}" data-detail-tab="notes">${t('我的笔记')}</button><button class="tab${detailActiveTab === 'transcript' ? ' active' : ''}" data-detail-tab="transcript">${t('字幕')}</button></div><div class="tabbar-extra">${action}${detailActiveTab === 'transcript' && !d.transcriptEditing ? renderRefineStatus(d) : ''}</div></div>`;
}
