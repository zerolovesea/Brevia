...
function renderMeetingDetail() {
  document.querySelector('.final-transcript').innerHTML = `<div class="tabbar"><button class="tab active" data-detail-tab="transcript">${t('逐字稿')}</button><button class="tab" data-detail-tab="refined">${t('精修字稿')}</button></div><div class="transcript-body" data-detail-panel="transcript">${uiData.detail.transcript.map(renderTranscriptSegment).join('')}</div><div class="transcript-body" data-detail-panel="refined" hidden>${uiData.detail.refinedTranscript.map(({ speaker, text }) => `<article class="segment"><div class="segment-meta"><span class="segment-speaker">${escapeHtml(formatSpeakerName(speaker))}</span></div><div class="segment-copy"><p>${escapeHtml(text)}</p></div></article>`).join('') || `<p class="refined-transcript-empty">${t('完成精修后，这里会显示不带时间戳的校对稿。')}</p>`}</div>`;
