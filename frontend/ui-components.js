...
function renderMeetingRow({ id, tone, title, meta, tags, status, deleted = false }, index) {
  const menu = deleted ? `<button data-meeting-action="restore" data-meeting-index="${index}">恢复</button>` : `<button data-meeting-action="rename" data-meeting-index="${index}">重命名</button><button data-meeting-action="category" data-meeting-index="${index}">分类 <span>›</span></button><button data-meeting-action="export" data-meeting-index="${index}">导出</button><button class="meeting-menu-danger" data-meeting-action="delete" data-meeting-index="${index}">删除</button>`;
  return `<article class="meeting-row" data-view="detail" data-meeting-index="${index}"${id ? ` data-meeting-id="${escapeHtml(id)}"` : ''}><div class="meeting-color ${tone}"></div><div class="meeting-main"><h2>${escapeHtml(title)}</h2><p>${escapeHtml(meta)}</p>${tags.map((tag) => `<div class="tag">${escapeHtml(tag)}</div>`).join('')}</div><div class="meeting-status"><span class="status ${status.tone}">${escapeHtml(status.label)}</span><small>${escapeHtml(status.detail)}</small></div><div class="meeting-actions"><button class="more" data-meeting-menu="${index}" aria-label="更多操作" aria-expanded="false">•••</button><div class="meeting-menu" hidden>${menu}</div><div class="meeting-rename-menu" hidden><form data-rename-meeting data-meeting-index="${index}"><label>重命名<input name="title" value="${escapeHtml(title)}" maxlength="120" required /></label><span><button data-cancel-rename type="button">取消</button><button type="submit">保存</button></span></form></div><div class="meeting-category-menu" hidden><button class="meeting-menu-back" data-meeting-action="back">← 分类</button><button data-assign-category="" data-meeting-index="${index}">未分类</button>${categories.map((category) => `<span><button data-assign-category="${escapeHtml(category)}" data-meeting-index="${index}">${escapeHtml(category)}</button><button class="meeting-category-delete" data-delete-meeting-category="${escapeHtml(category)}" aria-label="删除 ${escapeHtml(category)}">×</button></span>`).join('')}<form data-new-meeting-category><input name="category" maxlength="32" placeholder="新分类名称" required /><button type="submit">添加</button></form></div></div></article>`;
}
...
function renderModelRow({ icon, name, detail, intro = '' }) {
  return `<div class="model-row"><span class="model-icon">${icon}</span><div><b>${name}</b><small>${detail}</small>${intro ? `<small>${intro}</small>` : ''}</div><span class="status complete">${t('可用')}</span></div>`;
}
...
function renderStaticViews() {
  ...
  document.querySelector('#settings-view .settings-grid').innerHTML = `<section class="settings-card" id="installed-models"><h2>${t('模型库')}</h2>${uiData.settings.models.map(renderModelRow).join('')}<button class="secondary" data-settings-modal="models">${t('管理模型库')}</button></section>${uiData.settings.cards.map(renderSettingsCard).join('')}`;
}
